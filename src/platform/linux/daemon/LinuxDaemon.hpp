...
struct LinuxDaemonServices {
  ...
  LinuxDaemonServices();
  ~LinuxDaemonServices();
  LinuxDaemonServices(LinuxDaemonServices &&) noexcept;
  LinuxDaemonServices &operator=(LinuxDaemonServices &&) noexcept;
  LinuxDaemonServices(const LinuxDaemonServices &) = delete;
  LinuxDaemonServices &operator=(const LinuxDaemonServices &) = delete;
};

class LinuxDaemon final : public Daemon {
public:
  explicit LinuxDaemon(LinuxDaemonServices services);
  ~LinuxDaemon() override;

  int run() override;

private:
  LinuxDaemonServices services_;
};
