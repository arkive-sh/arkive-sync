...
MacosAtomicFileWriter::MacosAtomicFileWriter(std::filesystem::path finalPath)
  ...
  fd_ = ::open(tempPath_.c_str(), O_CREAT | O_TRUNC | O_WRONLY | O_CLOEXEC,
               0600);
  if (fd_ < 0) {
...
MacosAtomicFileWriter::~MacosAtomicFileWriter() { rollback(); }

void MacosAtomicFileWriter::preallocate(std::uint64_t size) {
  if (::ftruncate(fd_, static_cast<off_t>(size)) != 0) {
    throw sysError("ftruncate failed");
  }
}

void MacosAtomicFileWriter::writeAt(std::uint64_t offset,
...
void MacosAtomicFileWriter::commit() {
  if (::fsync(fd_) != 0) {
    throw sysError("fsync failed");
  }

  closeFd();
  std::filesystem::rename(tempPath_, finalPath_);

  const int dirFd = ::open(finalPath_.parent_path().c_str(), O_RDONLY);
  if (dirFd >= 0) {
    ::fsync(dirFd);
    ::close(dirFd);
  }
...
void MacosAtomicFileWriter::rollback() noexcept {
  if (fd_ >= 0) {
    ::close(fd_);
    fd_ = -1;
  }
...
void MacosAtomicFileWriter::closeFd() {
  if (fd_ >= 0) {
    if (::close(fd_) != 0) {
      fd_ = -1;
